...
| [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) | Standalone agent **runtime** with messaging gateways | **Integrated as the local runtime bridge (Part VI pick)** | Its memory/cron patterns are in identity §9/§12, and the runtime itself is now wired in natively: `src/lib/hermes.ts` talks to the user's own `hermes gateway` REST API (health/skills/toolsets/jobs/chat), Settings › ELION runtime › Hermes bridge manages it, and `hermes.*` tools (status, ask, skills.import, job.create, jobs.list) are registered behind `hermes.delegate` / `hermes.control` / `hermes.skills` permissions. Gateway offline = honest offline status, never simulated |
