...
test('MiniCPM unavailable state is honest and cannot send a fake reply', async ({ page }) => {
  ...
  await page.getByRole('radio', { name: /MiniCPM Desk Pet/ }).click()
  await page.getByRole('button', { name: 'Connect MiniCPM', exact: true }).click()
  await expect(page.locator('.nova-chat-error')).toContainText('could not be reached')
  await page.getByRole('textbox', { name: 'Message Elion', exact: true }).fill('Help me focus')
  await expect(page.getByRole('button', { name: 'Send message to Elion', exact: true })).toBeDisabled()
...
test('MiniCPM streaming contract drives Elion and keeps private context off by default', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Message Elion', exact: true }).fill('Help me find a next step')
  await page.getByRole('button', { name: 'Send message to Elion', exact: true }).click()
