...
test('MiniCPM unavailable state is honest and cannot send a fake reply', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Connect MiniCPM', exact: true }).click()
  await expect(page.locator('.nova-chat-error')).toContainText('MiniCPM is not connected')
  await page.getByRole('textbox', { name: 'Message Nova', exact: true }).fill('Help me focus')
  await expect(page.getByRole('button', { name: 'Send message to Nova', exact: true })).toBeDisabled()
  await expect(page.locator('.nova-message.from-assistant')).toHaveCount(0)
})

test('MiniCPM streaming contract drives Nova and keeps private context off by default', async ({ page }) => {
  let payload: any
  await page.route('**/api/companion/health', (route) =>
    route.fulfill({
      json: {
  ...
  await page.getByRole('button', { name: 'Open pet chat', exact: true }).click()
  await page.getByRole('button', { name: 'Connect MiniCPM', exact: true }).click()
  await expect(page.getByText('MiniCPM is connected', { exact: true })).toBeVisible()
  await page.getByRole('textbox', { name: 'Message Nova', exact: true }).fill('Help me find a next step')
  await page.getByRole('button', { name: 'Send message to Nova', exact: true }).click()
  await expect(page.locator('.nova-message.from-assistant')).toContainText('Protocol fixture response.')
...
test('the floating companion opens, moves with the keyboard, and can be unpinned', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Move Nova', exact: true }).focus()
  ...
  await page.getByRole('button', { name: 'Hide Nova', exact: true }).focus()
...
test('React Bits spotlight and dock move, with a real reduced-motion fallback', async ({ page }) => {
  ...
  await expect
    .poll(() => spotlight.evaluate((element) => element.style.getPropertyValue('--mouse-x')))
    .toContain('80')
