...
const activation = (page: Page) => page.getByRole('switch', { name: /Enable floating pet/ })
async function settings(page: Page) {
  await page.goto('/settings#companion')
  await expect(page.getByRole('heading', { name: 'Floating pet', exact: true })).toBeVisible()
}

test('the pet is an overlay, not a separate navigation page or dashboard card', async ({ page }) => {
  await page.goto('/')
...
test('Settings activation works immediately, persists, and closes any open chat', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Open pet chat', exact: true }).click()
  await expect(page.getByRole('dialog', { name: 'Nova chat', exact: true })).toBeVisible()
  await activation(page).click()
  await expect(pet(page)).toHaveCount(0)
  await expect(page.getByRole('dialog', { name: 'Nova chat', exact: true })).toHaveCount(0)
  await page.reload()
...
test('pet interactions animate locally without opening a page or invoking MiniCPM', async ({ page }) => {
  let modelRequests = 0
  ...
  await settings(page)
  const url = page.url()
  await pet(page).getByRole('button', { name: 'Interact with Nova', exact: true }).click()
  const controls = page.getByRole('dialog', { name: 'Nova controls', exact: true })
  await expect(controls).toBeVisible()
  await controls.getByRole('button', { name: 'Pat Nova', exact: true }).click()
  await expect(pet(page).locator('.nova-pet')).toHaveAttribute('data-pose', 'happy')
  await expect(pet(page).getByRole('status')).toContainText('Nova is happy')
  ...
  await controls.getByRole('button', { name: 'Let Nova nap', exact: true }).click()
  await expect(pet(page).locator('.nova-pet')).toHaveAttribute('data-pose', 'sleeping')
  await controls.getByRole('button', { name: 'Wake Nova', exact: true }).click()
  await expect(pet(page).locator('.nova-pet')).toHaveAttribute('data-pose', 'wave')
  await page.keyboard.press('Escape')
  await expect(controls).toHaveCount(0)
  await expect(pet(page).getByRole('button', { name: 'Interact with Nova', exact: true })).toBeFocused()
  expect(page.url()).toBe(url)
  expect(modelRequests).toBe(0)
})

test('the actual pet can be dragged without accidentally opening its menu', async ({ page }) => {
  await settings(page)
  const before = (await pet(page).boundingBox())!
  const avatar = (await pet(page)
    .getByRole('button', { name: 'Interact with Nova', exact: true })
    .boundingBox())!
  ...
  await expect(page.getByRole('dialog', { name: 'Nova controls', exact: true })).toHaveCount(0)
  await page.reload()
  await expect(pet(page)).toBeVisible()
  ...
  await pet(page).getByRole('button', { name: 'Interact with Nova', exact: true }).click()
  const panel = (await page.getByRole('dialog', { name: 'Nova controls', exact: true }).boundingBox())!
...
test('animation can be disabled separately while pet actions remain functional', async ({ page }) => {
  await settings(page)
  await page.getByRole('switch', { name: /Animate pet/ }).click()
  ...
  await expect(pet(page).locator('.nova-pet')).toHaveAttribute('data-pose', 'wave')
  await page.reload()
  await expect(page.getByRole('switch', { name: /Animate pet/ })).toHaveAttribute('aria-checked', 'false')
  await expect(pet(page).locator('.nova-pet')).toHaveAttribute('data-animated', 'false')
})

test('one pet follows the user into Lockdown and works on narrow screens', async ({ page }) => {
  ...
  await pet(page).getByRole('button', { name: 'Interact with Nova', exact: true }).click()
  await expect(page.getByRole('dialog', { name: 'Nova controls', exact: true })).toBeVisible()
  await page.getByRole('button', { name: 'Close pet controls', exact: true }).click()
  ...
  await expect(pet(page)).toBeVisible()
  await pet(page).getByRole('button', { name: 'Interact with Nova', exact: true }).click()
  const actions = (await page.getByRole('dialog', { name: 'Nova controls', exact: true }).boundingBox())!
  ...
  await page.getByRole('button', { name: 'Chat with Nova', exact: true }).click()
  const chat = (await page.getByRole('dialog', { name: 'Nova chat', exact: true }).boundingBox())!
  expect(chat.x).toBeGreaterThanOrEqual(0)
  expect(chat.x + chat.width).toBeLessThanOrEqual(375)
  await expect(page.getByRole('textbox', { name: 'Message Nova', exact: true })).toBeVisible()
})
